...
class SimpleTrainer(TrainerBase):
    ...
    def test_adv(self, split=None):
        ...
        #     transforms.ToTensor()
        # ])
        # image_dataset = datasets.ImageFolder(self.cfg.DATASET.TEST_PATH, img_transform)
        ...
        temp_model = get_model(self.model)
        surrogate = ClipModel(model=get_model(temp_model.image_encoder), num_classes=2).to(self.device)
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            input *= std
            input += mean
            images_adv = attacker.run(surrogate, input, scaler=1, feature_layer='fc')
            images_adv = normalize(images_adv)

            output = self.model_inference(images_adv)
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch_adv(self):
        ...
        temp_model = get_model(self.model)
        surrogate = ClipModel(model=temp_model.image_encoder, num_classes=2).to(self.device)
        attacker = PGD(8 / 255., preprocess=normalize, num_iters=10)
